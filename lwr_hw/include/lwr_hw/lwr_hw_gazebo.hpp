...
public:
  ...
  void write(ros::Time time, ros::Duration period)
  {
    ...
    case JOINT_IMPEDANCE:
      ...
      // for (int j = 0; j < n_joints_; j++)
      // {
      //   std::cout << joint_effort_command_[j]  << ", ";
      // }
      // std::cout << std::endl;
      // //
      // std::cout << "Sono in impedance control" << std::endl;
      for (int j = 0; j < n_joints_; j++)
      {
        ...
        // std::cout << "j_c_d" << j << ": " << joint_position_command_[j] << ", ";
        // double stiffness_effort = joint_stiffness_command_[j]*( joint_position_command_[j] - joint_position_[j] );
        //
        double stiffness_effort = 0.0;
        double effort = stiffness_effort + joint_effort_command_[j] + gravity_effort_(j);
